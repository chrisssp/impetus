# Wallpaper Bridge Specification

## Purpose

Kotlin MethodChannel bridge exposing `setBitmap` to Flutter for applying PNG images as lock-screen and system wallpapers. Part 0 delivers a spike-only test path; the contract is designed for Part 1+ reuse with rendering engine and scheduler integration.
...
- **MethodChannel contract**: `flutter test` — unit test mocks MethodChannel, verifies argument passing
- **setBitmap success**: Manual device test — photo proof on emulator (stock Android 13/14) + Xiaomi
- **Error handling**: `flutter test` — unit test sends invalid data, asserts error code
- **Spike test path**: Manual — tap button, verify lock screen via photo
